...
| Title | 23 to 38, line at 41 | Message title, white (light grey before 0.0.24), grey line under it from x 4 to 235; scrolls horizontally when wider than 232 px (F-005, D-037) |
...
- **Behavior:**
  ...
  - Fonts (since 0.0.15, D-031): each board names four u8g2 fonts in its `display.h` (top bar, title, small, large); the T-Display uses `helvR12` for the first three and `helvR18` for large. Row heights (bar, title, value area, countdown box) and the setup and welcome screens are laid out from the fonts' measured metrics at boot. Before 0.0.15: TFT_eSPI font 2 (16 px line) and font 4 (26 px). The Waveshare AMOLED uses `helvR14` (bar), `fub20` (title, bold; `helvR18` before 0.0.24), `helvR18` (small), and `helvR24` (large).
  - Colors: white, blue (0x4C9F, lighter than TFT_BLUE for readability on black), green, red. Since 0.0.24 "white" in the value is drawn light grey (198, 195, 198), so the white title stands out (D-037).
  - Title (since 0.0.24, D-037): white, with a thin grey line (66, 66, 66) under it as wide as the title area; a long title scrolls within that width. On rounded boards the title starts clear of the corner (x 13 on the Waveshare AMOLED), 8 px below the top bar, and the value starts 5 px below the line (AMOLED: title glyphs y 42 to 61, line y 74). Before 0.0.24: light grey title, dark grey (0x7BEF) line across the full width.
