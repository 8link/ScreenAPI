...
- **Behavior (since 0.0.12, D-028):**
  ...
  - On boards with rounded display corners (D-036) the whole bar moves down and in by the corner margin (7 px on the Waveshare AMOLED).
...
| D-036 | Rounded display corners: `board::kCornerRadius` per board; the top bar and the countdown box move in from the corners by R * (1 - 1/sqrt(2)) + 1 px. Serial command `C` shows a corner test (arcs of 20 to 70 px) to measure R. Waveshare AMOLED: 20 px (the 20 px arc is fully visible, so R is at most 20); T-Display and template: 0. | The user reported the AMOLED's rounded corners cutting off the top bar and the countdown. Waveshare does not document the radius. | 2026-09-24 |
